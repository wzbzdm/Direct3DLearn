...
// 默认配置构造函数
//Sampler::Sampler(Graphics& gfx) : parent() {
...
// 自定义配置构造函数
Sampler::Sampler(Graphics& gfx, Shape3DBase& parent) :gfx(gfx), parent(parent) {
    parent.AddObserver(this);
    InitSampler(gfx, parent.GetSamplerConf());
}
...
void Sampler::OnUpdate() {
    try {
        InitSampler(gfx, parent.GetSamplerConf()); // 更新配置
    }
